...
import pandas as pd
from muscleMapper_utils import k_fold_cross_val, dataset_TVTsplit
""""
...
for i in range(0, 5):
  ...
  """
  slice_train = np.array(slice_train)
  slice_test = np.array(slice_test)
  slice_val = np.array(slice_val)
  masks_train = np.array(masks_train)
  masks_test = np.array(masks_test)
  masks_val = np.array(masks_val)

  print("unique mask; ", len(masks_train))
  ...
  ratio = []
  for int in range(len(masks_train)):
    no_of_0s = (masks_train[int] == 0).sum()
    no_of_1s = (masks_train[int] == 1).sum()
  ...
  segment_pred_slb =  test_predictions #np.logical_and(test_predictions, bone_masks_test[:,np.newaxis,...])
  ...
  rows = 2
  columns = 5
  for b in range(1, 10): #len(c3s)):
    ax.append(fig.add_subplot(rows,columns, b)) #+1))
    #segment_pred_slb[i][segment_pred_slb[i]==0] = np.nan #uncomment to get pretty images
    plt.imshow(c3s[b,0,...], cmap="gray")
    plt.imshow(segment_pred_slb[b,0,...], cmap = "autumn", alpha = 0.5)
    ax[-1].set_title("Network test:"+str(b))
    plt.axis("off")
    #plt.show()
  #%%
  ...
  for k in range(0, len(ids)):
    if any(ids[k] == j for j in ids_test) == True:
      test_index.append(k)
      print(k)
  print(test_index)
  #%%
  ...
  feature_list_net = []
  feature_list_ours = []
  for l in range(0,len(ct_scans)):
    feature_list_net.append([extractionDict[a](ct_scans[l], segment_pred_slb[l], pixel_area_id[l]) for a in feat_list])
