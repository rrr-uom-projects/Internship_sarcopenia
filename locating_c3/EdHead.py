...
class headHunter_multiHead_deeper(nn.Module):
    def __init__(self, filter_factor=2):
        super(headHunter_multiHead_deeper, self).__init__()
        ff = filter_factor # filter factor (easy net scaling)
        # Input --> (3, 48, 120, 120)
        # conv layers set 1 - down 1
        self.c1 = nn.Conv3d(in_channels=3, out_channels=int(16*ff), kernel_size=3, padding=1)
...
class pred_head(nn.Module):
    def __init__(self, ff=2):
        ...
        self.drop10 = nn.Dropout3d(p=0.5)
        # prediction convolution
        self.pred = nn.Conv3d(in_channels=int(16*ff), out_channels=1, kernel_size=1)
...
class spineSeeker(nn.Module):
    def __init__(self, filter_factor=2):
        super(spineSeeker, self).__init__()
        ff = filter_factor # filter factor (easy net scaling)
        # Input --> (3, 48, 120, 120)
        # conv layers set 1 - down 1
        self.c1 = nn.Conv3d(in_channels=3, out_channels=int(16*ff), kernel_size=3, padding=1)
        ...
        self.bn_r2 = nn.BatchNorm3d(int(32*ff))
        self.drop_r2 = nn.Dropout3d(p=0.5)
        # separate final up block and prediction heads
        self.pred_head_1 = pred_head(ff=ff)
        self.pred_head_2 = pred_head(ff=ff)
        self.pred_head_3 = pred_head(ff=ff)
        self.pred_head_4 = pred_head(ff=ff)
        self.pred_head_5 = pred_head(ff=ff)
    ...
    def forward(self, im):
        ...
        # Up block 2 + predict heads
        x = torch.cat((self.pred_head_1(x, skip1), self.pred_head_2(x, skip1), self.pred_head_3(x, skip1), self.pred_head_4(x, skip1), self.pred_head_5(x, skip1)), axis=1)
        return x
